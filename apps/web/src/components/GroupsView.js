            ...
            <div class="card" style="margin: 0; display: flex; flex-direction: column; justify-content: space-between; border-right: ${g.is_section ? '4px solid #7c3aed' : '4px solid var(--centrly-primary)'};">
              <div>
                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.75rem;">
                  <div>
                    <h3 style="font-size: 1.05rem; font-weight: 800; color: var(--centrly-ink); margin: 0;">${escapeHtml(g.name)}</h3>
                  ...
                  <span class="badge badge-blue">${escapeHtml(g.centerName || g.center_name || 'السنتر')}</span>
                </div>
                <div style="font-size: 0.825rem; color: var(--centrly-text); margin-bottom: 0.5rem;">
                  📅 الموعد: <b>${escapeHtml(g.schedule || 'حسب الجدول')}</b>
                </div>
                <div style="font-size: 0.825rem; color: var(--centrly-text); margin-bottom: 0.5rem;">
                  👥 عدد الطلاب: <b>${escapeHtml(g.studentCount ?? g.students_count ?? 0)} طالب</b>
                </div>
                <div style="font-size: 0.825rem; color: var(--centrly-text);">
                  💵 سعر الحصة: ${isAssistant ? '<span class="badge badge-secondary">🔒 محجوب للمساعد</span>' : `<b>${escapeHtml(g.price ?? 0)} ج.م</b>`}
                </div>
              </div>
              ...
              <div style="display: flex; gap: 0.35rem; margin-top: 1.25rem; border-top: 1px solid var(--centrly-line); padding-top: 0.75rem; flex-wrap: wrap;">
                <button class="btn btn-primary btn-sm" style="flex: 1;" onclick="window.centrlyApp.startSessionForGroup('${escapeHtml(g.id)}')">
                  ⚡ بدء الحصة
                </button>
                <button class="btn btn-secondary btn-sm" onclick="window.centrlyApp.viewGroupDetails('${escapeHtml(g.id)}')">
                  عرض الطلاب
                </button>
                ${!g.is_section && !isAssistant ? `
                  <button class="btn btn-secondary btn-sm" onclick="window.centrlyApp.openCreateSectionModal('${escapeHtml(g.id)}')" title="إضافة سكشن فرعي">
                    ➕ سكشن
                  </button>
                  <button class="btn btn-secondary btn-sm" onclick="window.centrlyApp.viewGroupRollUp('${escapeHtml(g.id)}')" title="تقرير الإجمالي للدفعة">
                    📊 الإجمالي
                  </button>
                ` : ''}
              </div>
