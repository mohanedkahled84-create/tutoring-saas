      ...
      <div class="card" style="margin: 0; border-top: 4px solid var(--centrly-danger);">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-top: 1.25rem; padding-top: 1rem; border-top: 1px solid var(--centrly-line);">
          <div style="background: #fef2f2; padding: 0.75rem 1rem; border-radius: 8px; border-right: 4px solid #ef4444;">
            ...
            <div style="font-size: 1.5rem; font-weight: 900; color: #b91c1c; margin-top: 0.25rem;">${highCount} طلاب</div>
          </div>
          <div style="background: #fffbeb; padding: 0.75rem 1rem; border-radius: 8px; border-right: 4px solid #f59e0b;">
            ...
            <div style="font-size: 1.5rem; font-weight: 900; color: #d97706; margin-top: 0.25rem;">${mediumCount} طلاب</div>
          </div>
          <div style="background: #f0fdf4; padding: 0.75rem 1rem; border-radius: 8px; border-right: 4px solid #10b981;">
            ...
            <div style="font-size: 1.5rem; font-weight: 900; color: #15803d; margin-top: 0.25rem;">${students.length} طلاب</div>
          </div>
        </div>
      </div>
      ...
      <div class="card" style="margin: 0;">
        <div class="card-header">
          <h3 class="card-title" style="font-size: 1.05rem;">
            📋 كشف الطلاب المحتاجين للمتابعة (${students.length})
          </h3>
            ...
            <tbody>
              ${students.length > 0 ? students.map(s => `
                <tr>
                  ...
                  <td style="font-weight: 700;">${s.name || s.student_name}</td>
                  <td><span class="badge badge-blue">${s.group || s.group_name || 'مجموعة عامة'}</span></td>
                  <td>
                    <span style="font-weight: 700; color: var(--centrly-ink);">
                      ${s.alert_type === 'absence_warning' ? '❌ غياب متكرر' : (s.alert_type === 'grade_drop' ? '📉 تراجع درجات' : (s.alert_type === 'homework_neglect' ? '📝 إهمال واجب' : (s.reason || 'إنذار متابعة')))}
                    </span>
                  </td>
                  <td>
                  ...
                  <td style="font-size: 0.85rem; color: var(--centrly-text);">
                    ${s.reason || s.detail || 'تكرار الغياب عن الحصص الأخيرة'}
                  </td>
                  <td>
                    <button class="btn btn-secondary btn-sm" onclick="window.centrlyApp.dispatchSingleRiskAlert('${s.id || s.student_id}', '${s.name || s.student_name}')" style="font-size: 0.78rem; padding: 0.3rem 0.6rem;">
                      💬 إرسال إنذار واتساب
                    </button>
                  </td>
                </tr>
