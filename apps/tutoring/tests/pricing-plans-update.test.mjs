...
test('PRICING-PLANS: BillingService resolves new standard tiers (300, 750, 1500)', async () => {
  ...
  // 4. Proof amount detection: 7670 yearly (799 * 12 * 0.8) -> 750 students
...
test('PRICING-PLANS: AdminOpsService proof approval and override for new standard tiers', async () => {
  ...
  repo.paymentProofs = [
    ...
    },
    {
      id: 'proof-12470',
      tenant_id: 'tenant-adm-1',
      amount: 12470, // 1299 * 12 * 0.8 yearly
      status: 'pending',
  ...
  // Approve 399 monthly proof -> activates 300 students plan
  const res1 = await service.approvePaymentProof('proof-399', 'admin-1', 30);
  ...
  // Approve 12470 yearly proof -> activates 1500 students plan
  const res2 = await service.approvePaymentProof('proof-12470', 'admin-1', 365);
