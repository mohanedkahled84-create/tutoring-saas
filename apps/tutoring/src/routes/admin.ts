...
adminRouter.get("/tenants", async (_req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  try {
    const { data: tenants, error } = await supabase
      .from("tenants")
      .select(
        "id, name, status, subscription_status, trial_ends_at, subscription_ends_at, deleted_at, created_at"
      )
  ...
  async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    try {
      let query = supabase
        .from("payment_proofs")
        .select(
          "id, tenant_id, amount, payment_method, reference_number, proof_image_url, status, admin_notes, created_at, tenants(name)"
        )
        .order("created_at", { ascending: false });
    ...
    try {
      ...
      if (extend_days && typeof extend_days === "number") {
        ...
        updatePayload.subscription_ends_at = new Date(
          base.getTime() + extend_days * 24 * 60 * 60 * 1000
        ).toISOString();
