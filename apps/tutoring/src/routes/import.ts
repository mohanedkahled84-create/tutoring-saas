...
importRouter.post("/:id/students/import", async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  if (Array.isArray(directRows) && directRows.length > 0) {
    ...
    res.status(400).json({
      error: { code: "BAD_REQUEST", message: "Either 'rows' array or 'csv_content' text must be provided" },
    });
    return;
  }

  if (rawRows.length === 0) {
    res.status(400).json({ error: { code: "BAD_REQUEST", message: "Import payload contains no data rows" } });
    return;
  }
  ...
  try {
    ...
    for (let i = 0; i < rawRows.length; i++) {
      ...
      if (mapped.fee_override !== undefined && mapped.fee_override !== null && mapped.fee_override !== "") {
      ...
      if (mapped.exempt !== undefined && mapped.exempt !== null) {
        ...
        if (exemptVal === "true" || exemptVal === "1" || exemptVal === "نعم" || exemptVal === "معفي") {
          isExempt = true;
        }
      }
      ...
      if (enrollErr) {
        errors.push({ row: rowNum, name, error: `Student created but failed to link to group: ${enrollErr.message}` });
        continue;
      }
    ...
    res.status(500).json({ error: { code: "INTERNAL_ERROR", message: "Bulk import execution failed" } });
  }
});
